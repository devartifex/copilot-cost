...
import { migrateFromSqliteIfPresent } from "./otel/migrate.js";
...
async function cmdMigrate(): Promise<void> {
  const result = await migrateFromSqliteIfPresent();
  console.log(JSON.stringify(result, null, 2));
}

let dashboardMigrationStarted = false;

async function migrateBeforeDashboard(): Promise<void> {
  if (dashboardMigrationStarted) return;
  dashboardMigrationStarted = true;
  const result = await migrateFromSqliteIfPresent();
  if (result.exported > 0) {
    console.log(`migrated ${result.exported} legacy rows to ${result.jsonlPath}; archived ${result.dbPath} to ${result.archivedTo}`);
  }
}
