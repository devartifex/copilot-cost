...
export interface NormalizedCall {
  ...
  usd_cost: number;
  duration_ms: number;
  source: "cli-span" | "chat-logrecord" | "legacy-snapshot" | "unknown";
}
...
export function isChatSpan(record: unknown): boolean {
  ...
  if (record.source === "legacy-snapshot") return true;
...
export function normalizeSpan(record: unknown): NormalizedCall | null {
  ...
  const isLegacy = record.source === "legacy-snapshot";
  ...
  const freshInput = isLegacy ? rawInput : Math.max(rawInput - cacheRead, 0);
  ...
  const source: NormalizedCall["source"] = isLegacy ? "legacy-snapshot" : isLogRecord ? "chat-logrecord" : record.traceId || record.spanId || record.span_id ? "cli-span" : "unknown";
  ...
  const usdCost = source === "legacy-snapshot" ? num(record.usd_cost) : price ? computeCost({ input: freshInput + cacheRead + cacheCreation, cache_read: cacheRead, cache_write: cacheCreation, output }, price) : 0;
