...
export interface SessionDetail { session_id: string; snapshots: any[]; llm_calls: NormalizedCall[] }
...
export function sessionDetail(calls: NormalizedCall[], sessionId: string): SessionDetail {
  ...
  return {
    ...
    snapshots: scoped.filter((call) => call.source === "legacy-snapshot" && (call as any).snapshot).map((call) => (call as any).snapshot),
    llm_calls: scoped,
  };
}
